...
app.get("/api/v1/users/:user_id", async (req, res) => {
    ...
    try{
        const user = await sql
            "select * from users left join (select id_user, COUNT(*), TRUNC(AVG(rating),1) as average_rating from reviews group by id_user) reviews on users.user_id = reviews.id_user where id = "+req.params.user_id
